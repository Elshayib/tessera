//! CPU capture of a Still and a Turntable from Clay. The live GPU Viewport
//! is #11; until then the View recorder raymarches so a friend can open a
//! picture and an orbit without Tessera.
...
fn render(clay: Option<&Clay>, azimuth: f32) -> Vec<u8> {
    let eye = [azimuth.sin() * 1.8, 0.45, azimuth.cos() * 1.8];
    let mut rgb = Vec::with_capacity((SIZE * SIZE * 3) as usize);
    for y in 0..SIZE {
        for x in 0..SIZE {
            let color = match clay {
                Some(clay) => shade(clay, x, y, eye),
                None => SKY_RGB,
            };
...
fn shade(clay: &Clay, x: u32, y: u32, eye: [f32; 3]) -> [u8; 3] {
    let (origin, dir) = camera_ray(x, y, eye);
    match march(clay, origin, dir) {
        None => SKY_RGB,
        Some(p) => {
            let n = normal(clay, p);
            ...
            [
                (CLAY_RGB[0] as f32 * k) as u8,
                (CLAY_RGB[1] as f32 * k) as u8,
                (CLAY_RGB[2] as f32 * k) as u8,
            ]
        }
    }
}

fn camera_ray(x: u32, y: u32, eye: [f32; 3]) -> ([f32; 3], [f32; 3]) {
    let forward = normalize([-eye[0], -eye[1], -eye[2]]);
    let right = normalize(cross(forward, [0.0, 1.0, 0.0]));
    let up = cross(right, forward);
    let u = (x as f32 + 0.5) / SIZE as f32 - 0.5;
    let v = 0.5 - (y as f32 + 0.5) / SIZE as f32;
    let fov = 0.9;
    let dir = normalize([
...
fn march(clay: &Clay, origin: [f32; 3], dir: [f32; 3]) -> Option<[f32; 3]> {
    let mut t = 0.0;
    for _ in 0..64 {
        let p = [
            origin[0] + dir[0] * t,
...
fn normal(clay: &Clay, p: [f32; 3]) -> [f32; 3] {
    let e = 0.002;
    normalize([
        clay.sample([p[0] + e, p[1], p[2]]) - clay.sample([p[0] - e, p[1], p[2]]),
        clay.sample([p[0], p[1] + e, p[2]]) - clay.sample([p[0], p[1] - e, p[2]]),
        clay.sample([p[0], p[1], p[2] + e]) - clay.sample([p[0], p[1], p[2] - e]),
    ])
}
